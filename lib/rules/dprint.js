...
const dprint_1 = require("../util/dprint");
...
function* diff(s0, s1) {
    const changes = diff_1.diffChars(s0, s1);
    ...
    while (index <= lastIndex) {
        ...
        if (added) {
            ...
            if (next && next.removed) {
                ...
                const range = [cursor, cursor + next.value.length];
                index += 2;
            ...
            if (next && next.added) {
                // Handle it as replaced.
                const range = [cursor, cursor + text.length];
                index += 2;
                cursor += text.length;
                yield {
                    range,
...
exports.dprint = rule_1.rule({
    ...
    create: (context, options) => ({
        Program() {
            ...
            // Load dprint
            const { TypeScriptPlugin, formatFileText, resolveConfiguration, } = dprint_1.loadDprint(filePath);
            // Does format
            ...
            const formattedText = formatFileText({ filePath, fileText, plugins: [tsPlugin] });
...
Object.assign(module.exports, exports.dprint);
